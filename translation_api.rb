...
class CombinedLocalesUpdater
    ...
    Dir.chdir('./') do
      ...
      download_locales do |zip_file|
        locale_paths.each do |module_dir|
          extract_doc_locales(module_dir, zip_file)
        end
      end
    end
  end
  ...
  ##
  # Extract translated files from the zip
  def extract_doc_locales(module_dir, zip_file)
    directories = Dir.glob("#{module_dir}**/")
    # print directories
    directories.each do |directory|
      mod_name = get_crowdin_name(directory)
      path = versioned_path(mod_name).chomp("/")
      entries =  zip_file.glob("*/#{path}/*.html.md")
      Dir.chdir(module_dir) { replace_doc_locales(directory, entries) }
    end
  end
  ...
  def replace_locales(mod_path, entries)
    ...
    entries.each do |entry|
      ...
      replace_doc_file(target_directory, entry)
    end
  end

  ##
  # Override local crowdin locales
  def replace_doc_locales(mod_path, entries)
    mod_name = get_crowdin_name(mod_path)

    target_directory = mod_name
    unless File.directory?(target_directory)
      FileUtils.mkdir_p(target_directory)
    end

    # Clear all locales before checking in the current ones
    FileUtils.rm_f Dir.glob("#{target_directory}/*.*.html.md")
  ...
  ##
  # Replace a single doc translation file from crowdin
  def replace_doc_file(target_directory, entry)
    ...
    language_key = entry.name.split('/').first
    ...
    new_filename = "index.#{language_key}.html.md"
  ...
  ##
  # Upload all folders in the given locales_path.
  # Checks for index.html.md files.
  def upload_doc_translations(path)
    ...
    directories.each do |directory|
      ...
      if directory.include?("release-notes")
        puts "-> Skipping release notes folders"
        next
      end

      if mod_name == "core"
        puts "-> Skipping root folders"
        next
      end
      ...
      unless File.exists?("#{directory}/index.html.md")
        puts "-> Skipping #{mod_name} because index.html.md not present"
        next
      end
      ...
      upload_english mod_name,
                    ...
                    'index.html.md',
                    "Module #{mod_name.capitalize}",
                    'index.%two_letters_code%.html.md'
  ...
  def get_crowdin_name(path)
    if match = path.match(/([^\/]+)\/config\/locales/)
      match[1]
    elsif match = path.match(/\/source\/localizable\/(.+)/)
      match[1]
    else
      'core'
    end
...
updater = CombinedLocalesUpdater.new project: ENV['OPENPROJECT_CROWDIN_PROJECT'],
                                         ...
                                         locale_paths: ['./source/localizable/']
updater.call!
